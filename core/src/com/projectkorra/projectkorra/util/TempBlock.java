...
public class TempBlock {
	...
	private static final Map<Block, List<TempBlock>> INSTANCES = new HashMap<>();
	/**
	...
	public static TempBlock get(final Block block) {
		if (block != null) {
			List<TempBlock> tempBlocks = INSTANCES.get(block);
	...
	public static boolean removeBlock(final Block block) {
		List<TempBlock> blocks = INSTANCES.remove(block);
		if (blocks != null) {
	...
	private static void remove(TempBlock tempBlock) {
		List<TempBlock> blocks = INSTANCES.get(tempBlock.block);
		if (blocks != null) {
